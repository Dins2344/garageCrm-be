...
export const getGarage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    log.error('Failed to fetch garage', { garageId: req.user?.garage?._id, error: (error as Error).message });
    next(error);
  }
};
...
export const updateGarage = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    log.error('Failed to update garage', { garageId: req.user?.garage?._id, error: (error as Error).message });
    next(error);
  }
};
