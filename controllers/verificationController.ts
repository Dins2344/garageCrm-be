...
export const getStatus = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    log.error('Failed to read verification status', { userId: req.user?._id, error: (error as Error).message });
    next(error);
  }
};
...
export const sendCode = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    log.warn('Verification code request failed', { userId: req.user?._id, channel: req.params.channel, error: (error as Error).message });
    next(error);
  }
};
...
export const confirmCode = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    const channel = channelParam(req);
    ...
    log.warn('Verification confirm failed', { userId: req.user?._id, channel: req.params.channel, error: (error as Error).message });
    next(error);
  }
};
