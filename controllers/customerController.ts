...
export const getCustomers = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    log.error('Failed to fetch customers', { garageId: req.user?.garage?._id, error: (error as Error).message });
    next(error);
  }
};
...
export const createCustomer = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    log.error('Failed to create customer', { garageId: req.user?.garage?._id, error: (error as Error).message });
    next(error);
  }
};
