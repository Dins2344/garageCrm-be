...
exports.getJobCards = async (req, res, next) => {
  try {
    const { status, mechanicId, vehicle, search, page, limit } = req.query;
    ...
    log.error('Failed to fetch job cards', { garageId: req.user?.garage?._id, error: error.message });
    next(error);
  }
};
...
exports.getJobCard = async (req, res, next) => {
  try {
    ...
    log.error('Failed to fetch job card', { jobCardId: req.params.id, error: error.message });
    next(error);
  }
};
...
exports.createJobCard = async (req, res, next) => {
  try {
    ...
    const populated = await jobCardUsecase.getJobCardDetails({ jobCardId: jobCard._id, garageId });
    log.info('Job card created successfully', { jobCardId: jobCard._id, jobCardNumber: jobCard.jobCardNumber, garageId });
    res.status(201).json({ success: true, data: populated });
  } catch (error) {
    log.error('Failed to create job card', { garageId: req.user?.garage?._id, error: error.message });
    next(error);
  }
};
...
exports.updateJobCard = async (req, res, next) => {
  try {
    ...
    log.error('Failed to update job card', { jobCardId: req.params.id, error: error.message });
    next(error);
  }
};
...
exports.updateEstimation = async (req, res, next) => {
  try {
    ...
    log.error('Failed to update estimation', { jobCardId: req.params.id, error: error.message });
    next(error);
  }
};
...
exports.approveEstimation = async (req, res, next) => {
  try {
    ...
    log.error('Failed to approve estimation', { jobCardId: req.params.id, error: error.message });
    next(error);
  }
};
...
exports.deleteJobCard = async (req, res, next) => {
  try {
    ...
    log.error('Failed to delete job card', { jobCardId: req.params.id, error: error.message });
    next(error);
  }
};
...
exports.downloadEstimation = async (req, res, next) => {
  try {
    ...
    log.error('Failed to generate estimation PDF', { jobCardId: req.params.id, error: error.message });
    next(error);
  }
};
